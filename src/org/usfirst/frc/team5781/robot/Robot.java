...
 Spark m_frontRight = new Spark(3); Spark m_rearRight = new Spark(4); SpeedControllerGroup m_right = new SpeedControllerGroup(m_frontRight, m_rearRight);
 DifferentialDrive m_drive = new DifferentialDrive(m_left, m_right);
 // Define joystick being used at USB port 1 on the Driver Station Joystick m_driveStick = new Joystick(1);
 // Create a special gyro controller object ADXRS450_Gyro m_gyro = new ADXRS450_Gyro();
 // Keeps track of time state was entered Timer m_autonStateTimer;
 DoubleSolenoid m_dsol = DoubleSolenoid(0,1);
 public void autonomousInit() {  m_autonState = AutonState.AUTON_STATE_FORWARD;  m_autonStateTimer = new Timer();  m_autonStateTimer.start();
 }
 //To Be Filled with Periodic Stuff public void autonomousPeriodic() {   }
 public void robotInit() {  // Set the gyro controller to zero on init.  m_gyro.reset(); }  public double dumpInput(double x) {  if( x < 0.1 && x > -0.1 ) {   return 0;  }    int sign;  if( x < 0 ) {   sign = -1;  }  else {   sign =1;  }    x = 0.9*x + (0.1 * sign);   x = x * Math.abs(x);  returtn x; }
 public void teleopPeriodic() {  joystickDrive();  joystickButtons(); }
 public void joystickDrive() {  double x = dumpInput( m_driveStick.getX() );  double y = dumpInput( m_driveStick.getY() );  double z = dumpInput( m_driveStick.getTwist() );
